...
void loop()
{
    ...
    g_status = control_snapshot();
    if (g_status.enc_last_dir < 0)
    {
        if (g_status.enc_count != lastPrintedCount)
        {
            Serial.printf("CCW (%ld)\n", g_status.enc_count);
            lastPrintedCount = g_status.enc_count;
            Serial.print(F("> "));
        }
    }
    else if (g_status.enc_last_dir > 0)
    {
        if (g_status.enc_count != lastPrintedCount)
        {
            Serial.printf("CW (%ld)\n", g_status.enc_count);
